...
func (s *authService) GetSession(ctx context.Context, email string) (dto.GetSessionResponse, error) {
	...
	res := dto.GetSessionResponse{
		...
		Email:      user.Email,
		Phone:      user.Phone,
		Position:   enums.EmployeePositionMapIdx[user.Position.EmployeePositionIdx].LongLabel["id"],
		Department: enums.EmployeeDepartmentMapIdx[user.Department.EmployeeDepartmentIdx].LongLabel["id"],
		Salary:     user.Salary,
		Address:    user.Address,
		Status:     enums.EmployeeStatusMap[user.Status],
